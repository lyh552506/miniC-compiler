...
void Build(Function& function)
{
    ...
    for (auto block : function.bbs)
    {
             ...
             InstEnd++)
        {
            ...
            { // TODO 这一处的实现需要处理后的inst，来查询是否是mov指令
                // TODO live<---live\use(I)

                combine.insert(def);
                combine.insert(use.begin(),
                               use.end()); // 将mov指令的src和dst合并
                for (auto n : combine)
                { // 遍历这条指令的src和dst，并将这些结点加入到传送指令集合
...
void Simplify()
{
    ...
    for ( _Node& m : adjlist)
    {
      if(selectlist.find(m)==selectlist.end()&&coalescedNodes.find(m)==coalescedNodes.end()){
        DecrementDegree(m);
      }
        // auto it1 = std::find(selectlist.begin(), selectlist.end(), m);
        // auto it2 = std::find(coalescedNodes.begin(), coalescedNodes.end(), m);
        // if (it1 == selectlist.end() && it2 == coalescedNodes.end())
        // { // 使用迭代器进行查找，如果当前与node冲突的结点m不是可简化结点或者已合并的结点,则将他们的的degree减1（已简化和已合并结点不用再考虑度数）
        //     DecrementDegree(m);
        // }
    }
}
...
bool IsMoveRelated(const _Node& node)
{
    ...
    if (it != moveList.end() && !it->second.empty())
    { // 当node存在于movelist并且对应的instruction不为空，此时可以判定他是传送相关结点
        return true;
    }
    return false;
}
...
std::list<_Node> Adjacent(_Node& node)
{
    ...
    for(auto& Nod:L){
      auto it1 = std::find(selectlist.begin(), selectlist.end(), Nod);
      auto it2 = std::find(coalescedNodes.begin(), coalescedNodes.end(), Nod);
      if(it1!= selectlist.end()||it2!=coalescedNodes.end()){ //在这两个集合不为空
          L.remove(Nod);
      }
    }
    return L;
...
void DecrementDegree(_Node& node)
{
    ...
    node.storeDegree(Degree-1);
    if (Degree == K)
    {
      std::list<_Node> Adjlist=Adjacent(node);
      Adjlist.emplace_back(node);
      //EnableMoves();
      spillWorkLists.erase(node);//当使他可移动时，他们不再属于高度数结点
