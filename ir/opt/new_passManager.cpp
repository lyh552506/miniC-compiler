...
void _PassManager::RunOnLevel()
{
    ...
    if (level == O1)
    {
        ...
        while (modified)
        {
            modified = false;
            // mem2reg
            modified |= CommonPass(AM);
        }
        while (RunImpl<Inliner>(module, AM))
        {
            RunLevelPass(cfgSimplify, curfunc, modified) RunImpl<DeadArgsElimination>(module, AM);
            RunImpl<StoreOnlyGlobalElimination>(module, AM);
            RunImpl<Global2Local>(module, AM);
            modified = true;
            while (modified)
        ...
        // Loops
        {
            ...
            {
                modified = false;
                PassChangedBegin(curfunc) PassChangedEnd RunLevelPass(LoopSimplify, curfunc, other);
                ...
                PassChangedBegin(curfunc) PassChangedEnd
                    ...
                    RunLevelPass(LoopRotate, curfunc, other) PassChangedBegin(curfunc) PassChangedEnd
                        // licm
...
{
    bool modified = false;
    static bool HasCombineCond = false;

    RunLevelPass(cfgSimplify, curfunc, modified);
    PassChangedBegin(curfunc) PassChangedEnd PassChangedBegin(curfunc) RunImpl<Mem2reg>(curfunc, AM);
    PassChangedEnd 
    if (!HasCombineCond)
    {
    ...
    RunLevelPass(GepEvaluate, curfunc, modified);

    RunLevelPass(ConstantProp, curfunc, modified);
    RunLevelPass(DCE, curfunc, modified);
    RunLevelPass(BlockMerge, curfunc, modified);
    PassChangedBegin(curfunc) PassChangedEnd
        // TRE
        // RunLevelPass(TailRecurseEliminator, curfunc, modified) return modified;
    return modified;
}
    ...
    while (!EnablePass.empty())
    {
        ...
        default: {
            for (int i = 0; i < module->GetFuncTion().size(); i++)
            {
                ...
                if (func->tag == Function::UnrollBody || func->tag == Function::ParallelBody)
                    continue;
                curfunc = func.get();
                // 维护bbs关系
                curfunc->bb_num = 0;
