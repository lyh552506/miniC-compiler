...
Value *SimplifyDivInst(Value *LHS, Value *RHS) {
  ...
  if (LHS == RHS) {
    ...
    else if (dynamic_cast<ConstIRFloat *>(LHS) &&
             dynamic_cast<ConstIRFloat *>(RHS))
  ...
  if (auto Mul = dynamic_cast<BinaryInst *>(LHS)) {
    ...
    else if (Mul->getopration() == BinaryInst::Op_Mul &&
             Mul->GetOperand(0) == RHS)
...
Value *SimplifyIcmpInst(BinaryInst* inst, BinaryInst::Operation Opcode, Value *LHS, Value *RHS) {
  ...
  if (LHS == RHS && Opcode == BinaryInst::Op_NE)
    return ConstIRBoolean::GetNewConstant(false);
  // X != Y -> true
  if (LHS->isConst() && RHS->isConst() && Opcode == BinaryInst::Op_NE &&
      LHS != RHS)
    return ConstIRBoolean::GetNewConstant(true);
...
bool MatchAddModInst(BasicBlock *cur) {
  for (auto iter = cur->begin(); iter != cur->end();) {
    ...
    while (match) {
      match = false;
      if (bin && bin->getopration() == BinaryInst::Op_Add &&
          bin->GetUserListSize() == 1 && bin->GetOperand(1)->isConst()) {
        if (!Base)
          Base = bin;
        AddInst.push_back(bin);
        auto user = bin->GetUserlist().Front()->GetUser();
        auto mod = dynamic_cast<BinaryInst *>(user);
        if (mod && mod->getopration() == BinaryInst::Op_Mod &&
            mod->GetParent() == bin->GetParent() &&
            mod->GetUserListSize() == 1 && mod->GetOperand(1)->isConst()) {
          if (ModNum == 0)
            ModNum = dynamic_cast<ConstIRInt *>(mod->GetOperand(1))->GetVal();
          else {
            if (ModNum !=
                dynamic_cast<ConstIRInt *>(mod->GetOperand(1))->GetVal())
              break;
          }
          
          ModInst.push_back(mod);
          bin =
              dynamic_cast<BinaryInst *>(mod->GetUserlist().Front()->GetUser());
          match = true;
        } else {
          ...
          Sum = Sum % ModNum;
          auto New_Add = new BinaryInst(op, BinaryInst::Op_Add,
                                        ConstIRInt::GetNewConstant(Sum));
          auto New_Mod = new BinaryInst(New_Add, BinaryInst::Op_Mod,
                                        ConstIRInt::GetNewConstant(ModNum));
